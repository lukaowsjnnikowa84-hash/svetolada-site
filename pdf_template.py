...
class PDFTemplate(FPDF):
    def __init__(self):
        ...
        # Цветовая палитра Светолады (золотистая)
        self.colors = {
            "text": (58, 58, 58),          # #3A3A3A — мягкий графит
            "title": (168, 184, 255),      # #A8B8FF — сияющий лавандово-голубой
            "accent": (232, 215, 168),     # #E8D7A8 — светлое золото
            "line": (220, 196, 142),       # #DCC48E — тёплое золото
            "fallback_bg": (250, 245, 235) # мягкий кремовый фон, если нет картинки
        }
    ...
    def add_cover_page(self, name: str):
        ...
        # Фон туманности
        try:
            self.image("assets/images/nebula_gold.png", x=0, y=0, w=210, h=297)
        except:
            self.set_fill_color(*self.colors["fallback_bg"])
